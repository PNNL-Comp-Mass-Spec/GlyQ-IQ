...
namespace Sipper.ViewModel
{
    public class FileInputsViewModel : ViewModelBase
    {
        ...
        public string MsParameterFilePath
        {
            get { return _fileInputsInfo.MsWorkflowParameterFilePath; }
            set
            {
                if (value == _fileInputsInfo.MsWorkflowParameterFilePath)
                    return;

                _fileInputsInfo.MsWorkflowParameterFilePath = value;
                OnPropertyChanged("MSParameterFilePath");
            }
        }
        ...
        public void CreateFileLinkages(IEnumerable<string> fileNames)
        {
            ...
            //3.  pull out .rft file third
            var rtfFileNames = (from n in fileNames where Path.GetExtension(n) != null && Path.GetExtension(n) == ".rtf" select n);

            if (rtfFileNames.Any())
            {
                CreateFileLinkage(rtfFileNames.First());

                //remove all text files from inputs
                if (fileNames != null) fileNames = fileNames.Except(rtfFileNames);
            }

            //4.  anything else is a dataset
            if (fileNames.Any())
            {
        ...
        public void CreateFileLinkage(string fileOrFolderPath)
        {
            ...
            else
            {
                ...
                if (fileExtension != null)
                {
                    ...
                    {
                        MsParameterFilePath = fileOrFolderPath;
                    }
                    else
                    {
                        DatasetPath = fileOrFolderPath;
