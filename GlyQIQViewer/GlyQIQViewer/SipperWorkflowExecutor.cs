...
namespace Sipper
{
    public sealed class SipperWorkflowExecutor : TargetedWorkflowExecutor
    {
        ...
        public override void InitializeWorkflow()
        {
            ...
            Check.Require(WorkflowParameters is Sipper.SipperWorkflowExecutorParameters, "Parameters are not of the right type.");
            
            string db = ((Sipper.SipperWorkflowExecutorParameters)WorkflowParameters).DbName;
            string server = ((Sipper.SipperWorkflowExecutorParameters)WorkflowParameters).DbServer;
            string table = ((Sipper.SipperWorkflowExecutorParameters)WorkflowParameters).DbTableName;


            List<int> massTagIDsForFiltering =
                GetMassTagsToFilterOn(((Sipper.SipperWorkflowExecutorParameters)WorkflowParameters).TargetsToFilterOn).Distinct().ToList();
            ...
            else
            {
                ...
                MassTagsForReference = GetMassTagTargets(((Sipper.SipperWorkflowExecutorParameters)WorkflowParameters).ReferenceDataForTargets, MassTagIDsinTargets.Select(p => (int)p).ToList());
            }
        ...
        private void ReadResultsFromDB(TargetCollection targetCollection, DbDataReader reader)
        {
            ...
            while (reader.Read())
            {
                ...
                result.ElutionTimeUnit = Globals.ElutionTimeUnit.ScanNum;
