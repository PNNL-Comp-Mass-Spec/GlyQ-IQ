...
namespace GetPeaksDllLite.DataFIFO
{
    public class DataPeaksDataWriter : IPeakWriter
    {
        ...
        public int WriteOmicsProcessedPeakData(List<ProcessedPeak> XYDataList, int scanNumber, string path)
        {
            int isSuccess = 0;//nonsucessful outcome.  Still to be determined
        ...
        public static List<string> ConvertPeaksTOStingsPeak(List<ProcessedPeak> XYDataList, int scanNumber, string seperator)
        {
            List<string> finalList = new List<string>();

            int counter = 1;
            foreach (ProcessedPeak point in XYDataList)
            {
                string mass = SetPrecision(point.XValue, 5);
                string height = SetPrecision(point.Height, 4);
                string width = SetPrecision(point.Width, 6);
