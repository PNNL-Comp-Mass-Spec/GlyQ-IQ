...
namespace IQGlyQ
{
    public static class SelectClosest
    {
        ...
        public static Run64.Backend.Core.Peak SelectNearestPeakToCenter(List<Run64.Backend.Core.Peak> peaks, double peakCenter)
        {
            //processed Peaks must have a CenterIndexLeft

            Run64.Backend.Core.Peak selectedPeak = new Run64.Backend.Core.Peak();


            //Console.WriteLine("The center is close to " + peakCenter);
            if (peaks.Count > 1)//we missed the center scanset so we need to find it so we sync with decon scanset collection
            {
                List<Tuple<double, Run64.Backend.Core.Peak>> tuplePile = new List<Tuple<double, Run64.Backend.Core.Peak>>();

                for (int i = 0; i < peaks.Count; i++)
                {
                    Run64.Backend.Core.Peak currentPeak = peaks[i];
                    double difference = currentPeak.XValue - peakCenter;
                    tuplePile.Add(new Tuple<double, Run64.Backend.Core.Peak>(difference * difference, currentPeak));
                }

                List<Tuple<double, Run64.Backend.Core.Peak>> sortedTuplePile = tuplePile.OrderBy(r => r.Item1).ToList();
