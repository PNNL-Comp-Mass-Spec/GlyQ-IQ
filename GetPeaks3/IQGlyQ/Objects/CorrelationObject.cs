...
namespace IQGlyQ.Objects
{
    public class CorrelationObject
    {
        ...
        /// </summary>
        public List<Run64.Backend.Data.XYData> PeakChromXYData { get; set; }
        ...
        public CorrelationObject(IsotopicProfile iso, ScanObject scans, Run run, ref ProcessorChromatogram _lcProcessor, FragmentedTargetedWorkflowParametersIQ workflowParameters)
        {
            ...
            PeakChromXYData = new List<Run64.Backend.Data.XYData>();
            PeaksFromChromXYData = new List<List<ProcessedPeak>>();
            List<ProcessedPeak> peaksInEIC;
            Run64.Backend.Data.XYData clippedEIC;
            foreach (MSPeak peak in iso.Peaklist)
            {
                ...
                if (clippedEIC == null)
                {
                    clippedEIC = new Run64.Backend.Data.XYData();
                }
                if (peaksInEIC.Count == 0)
                {
                    peaksInEIC = new List<ProcessedPeak>();//add empty peak
        ...
        private Run64.Backend.Data.XYData CreateBufferSmoothClip(ScanObject scans, Run run, ref ProcessorChromatogram lcProcessor, MSPeak peak, ProcessingParametersChromatogram LCparameters)
        {
            //1.  return a raw chromatorgram with buffers on each side
            Run64.Backend.Data.XYData deconBufferedChromatogram = lcProcessor.GenerateBufferedChromatogramByPoint(run, scans, peak.XValue);
            ...
            //4.  convert back
            Run64.Backend.Data.XYData finallyData = ConvertProcessedPeakToXYData.ConvertPointsToDecon(newMethodInfinity);
            return finallyData;
        }

        //slower and should produce the same results as the buffered version
        private Run64.Backend.Data.XYData CreateFullSmoothClip(ScanObject scans, Run run, ref ProcessorChromatogram lcProcessor, MSPeak peak, ProcessingParametersChromatogram LCparameters)
        {
            //1.  return a raw chromatorgram with buffers on each side
            ...
            Run64.Backend.Data.XYData deconBufferedChromatogram = ProcessorChromatogram.DeconChromatogramGeneratorWrapper(run, peak.XValue, scans);
        ...
        private bool CheckEIC(bool isThereAPeakInFront, int minSizeOfEIC)
        {
            ...
            {
                Run64.Backend.Data.XYData EICofInterest = PeakChromXYData[i];
        ...
        private Run64.Backend.Data.XYData FilterOutDataBasedOnMsMsLevel(Run run, Run64.Backend.Data.XYData xyData, int msLevelToUse = 1, bool usePrimaryLcScanNumberCache = true)
        {
            if (xyData == null || xyData.Xvalues.Length == 0) return xyData;

            Run64.Backend.Data.XYData filteredXYData = new Run64.Backend.Data.XYData();
