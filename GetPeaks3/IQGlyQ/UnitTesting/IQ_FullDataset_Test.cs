...
namespace IQGlyQ.UnitTesting
{
    public class IQ_FullDataset_Test
    {
        ...
        public List<string> ExecuteMultipleTargetsTest2(string testFile, string peaksTestFile, string resultsFolder, string targetsFile, string executorParameterFile, string factorsFile, string loggingFolder, string XYDataFolder, string dataSetType, string pathFragmentParameters, string filefolderPath, string lockLocation, string coreID, bool singlecharge)
        {
            bool isUnitTest = false;//true for module testing only
            EnumerationIsPic isPic;
            //EnumerationDataset thisDataset;
            ...
            List<GlyQIqResult> glyQIqResultStrings = PostRun(deleteFiles, resultsFolder, expectedResultsFilename, expectedResultsFilenameSummary, lockLocation, coreID);

            Console.WriteLine(glyQIqResultStrings.Count + " results were found");
            
            List<string> simpleResultsOutput = new List<string>();
            foreach (var result in glyQIqResultStrings)
            {
                simpleResultsOutput.Add(result.GlyQIqResultToString());
            }
        ...
        private static List<GlyQIqResult> PostRun(bool deleteFiles, string resultsFolder, string expectedResultsFilename, string expectedResultsFilenameSummary, string lockLocation, string coreID)
        {
            ...
            Console.WriteLine("Lets look and see if old file exists: " + expectedResultsFilename);
            ...
            if (Directory.Exists(resultsFolder))
            {
                ...
                {
                    Thread.Sleep(5000);
                    Console.WriteLine("Deleting directory " + resultsFolder + " failed");
                    ...
                    {
                        Console.WriteLine("Failed again after 5 seconds");
                        string[] files = Directory.GetFiles(resultsFolder);
                        Console.WriteLine("There are " + files.Length + " files present in the directory we want to delete");
                        if (files.Length == 1)//safety
                        {
                            DeleteDirectory(resultsFolder);
                            throw;
                        }
                    }
                }
        ...
        private static void StopClock(DateTime starttime, Stopwatch stopWatch)
        {
            ...
            var elapsedMinutes = (int)Math.Floor(stopWatch.Elapsed.TotalMinutes);
            var elapsedSeconds = (int)((stopWatch.Elapsed.TotalMinutes - elapsedMinutes) * 60);

            Console.WriteLine("This took " + elapsedMinutes + " minutes, " + elapsedSeconds + " seconds to find the eluting peaks");
            Console.WriteLine("");
        }
        ...
        /// <summary>
        /// brute force delete
        /// //check to make sure this does not get wild on the C drive etc.
        /// </summary>
