...
{
    public class RemoveInsourceFragmentationIQ : TaskIQ
    {
        /// <summary>
        ...
        public RemoveInsourceFragmentationIQ(FragmentedTargetedWorkflowParametersIQ parameters, Run run, ProcessorChromatogram lcProcessor, ProcessorMassSpectra msProcessor, IGenerateIsotopeProfile theorFeatureGenV2, TheoreticalIsotopicProfileWrapper monster)
        
        {
            ...
            //Do we need to make another here singleton 2
           // ITheorFeatureGenerator _theorFeatureGen = new JoshTheorFeatureGenerator(parameters.IsotopeProfileType, parameters.IsotopeLowPeakCuttoff);//perhaps simple constructor
            ...
            MassProton = Globals.PROTON_MASS;
        ...
        public void CleanData(Run runIn, IqResult iQresultIn, EnumerationParentOrChild whichDirectionToLook, IsotopicProfile isoCorrespondingToTarget)
        {
        //setup
            ...
            if (iQresult.IqResultDetail.ChromPeakQualityData != null && iQresult.IqResultDetail.ChromPeakQualityData.Count > 0)
            {
                ...
                for (int i = rangeStart; i < rangeStop ; i++)//normal
                {
                    ...
                    //brind in unmodified iso here
                    iQresult.Target.TheorIsotopicProfile = isoCorrespondingToTarget.CloneIsotopicProfile();
                    ...
                    if(fragmentPassesProcessing)//10-28-2013
                    {
                        ...
                        noParentResult.FragmentFitAbundance = integratedArea;
                        noParentResult.Primary_Target.TheorIsotopicProfile.IntensityAggregateAdjusted = integratedArea;
                    ...
                    FragmentIQTarget chargedParentsFromAllFragmentsToSearchForMass = Utiliites.GenerateListOfCandidateParentsTestIQ(run, iQresult, possibleFragmentPeakQuality, peakQualityScanBounds, whichDirectionToLook, ref passWorkflowParameters, ref localTheorFeatureGenV2, ref errorlog, ref LcProcessor,ref localMonster);
                    ...
                    List<IqGlyQResult> finalChargedParentsFromAllFragmentsToSearchForLcResults = Utiliites.VerifyByTargetedFeatureFindingTestIQ(chargedParentsFromAllFragmentsToSearchForMass, fitScoreCuttoff, run, ref _msfeatureFinder, ref passFutureTargets, _workflowParameters.MSParameters.IsoParameters, MsProcessor, _workflowParameters.MSToleranceInPPM, ref errorlog);
                    ...
                    if (errorlog.Item2 == "Success" && glycanChildren.Count > 0 && fragmentPassesProcessing == true) //if the fragment failed, there is no points continuing
                    {
                        ...
                        {    
                            ...
                            //IGenerateIsotopeProfile localTheorFeatureGenV2 = TheorFeatureGenV2;
                            //TheoreticalIsotopicProfileWrapper.Generate(ref localTheorFeatureGenV2, targetParent, _workflowParameters.MSParameters.IsoParameters.IsotopeProfileMode, deltaMassCalibrationMZ, deltaMassCalibrationMono, toMassCalibrate, penaltyMode);
                            Monster.Generate(ref localTheorFeatureGenV2, targetParent, _workflowParameters.MSParameters.IsoParameters.IsotopeProfileMode, toMassCalibrate, toShift);
                    ...
                    else
                    {
                        ...
                        if(noParentResult.Primary_Observed_IsotopeProfile==null)
                        {
                            //this is the second of 2 placces to store isotope profiles for wxport
                            noParentResult.Primary_Observed_IsotopeProfile = new IsotopicProfile();
                            noParentResult.Primary_Observed_IsotopeProfile.Score = -1;
                        }
            ...
            Console.WriteLine("Finished with looping through lc peaks " + iQresult.ToChild.Primary_Target.Code + Environment.NewLine);
            ...
            if (print)
            {
                ...
                {
                    if (hit.Primary_Target != null && hit.Primary_Target.TheorIsotopicProfile != null)
                    {
                        Console.WriteLine(printcounter +
                                          " We found scan " + hit.Primary_Target.ScanLCTarget +
                                          " with mono mass " + hit.Primary_Target.TheorIsotopicProfile.MonoIsotopicMass +
                ...
                {
                    if (hit.Primary_Target != null && hit.TargetParent != null &&
                        hit.Primary_Target.DifferenceName != null && hit.TargetParent.TheorIsotopicProfile != null)
                    {
                        Console.WriteLine(printcounter +
                                          " We found scan " + hit.TargetParent.ScanLCTarget +
                                          " with mono mass " + hit.Primary_Target.TheorIsotopicProfile.MonoIsotopicMass +
                                          " charge " + hit.FragmentCharge +
                                          " is a fragment with a mass larger by " + hit.Primary_Target.DifferenceName +
