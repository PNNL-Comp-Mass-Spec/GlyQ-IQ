...
using MSPeakResult = Run64.Backend.DTO.MSPeakResult;

namespace Run64.Utilities
{
    public class PeakUtilities
    {


        public clsPeak2 lookupPeak(double mz, clsPeak2[] peaklist, double mzVar)
        {
        ...
        public int lookupPeakIndex(double mz, clsPeak2[] peaklist, double mzVar)
        {
            for (int i = 0; i < peaklist.Length; i++)
            {
                if (Math.Abs(mz - peaklist[i].mdbl_mz) <= mzVar)
        ...
        public static XYData GetChromatogram(List<MSPeakResult> peakList, double targetMZ, double toleranceInMZ)
        {
