...
namespace HammerPeakDetectorConsole
{
    ...
    {
        public static void DetectPeaksWithHammer(string dataFileName, string outputFolder, string outputPeaksFileName, Run32.Backend.Core.Run runIn, bool writeMetrics, int hammerMode)
        {
            PeakListTextExporter peakExporter = new PeakListTextExporter(runIn.MSFileType, outputPeaksFileName);
            ...
            for (int i = start; i < stop; i++)
            {
                ...
                Run32.Backend.Core.ScanSet LCScanSetSelected = runIn.ScanSetCollection.GetScanSet(currentScan);
                ...
                Run32.Backend.Data.XYData MassSpectrum = msProcessor.DeconMSGeneratorWrapper(runIn, LCScanSetSelected);
                List<PNNLOmics.Data.XYData> convertedMassSpectrum = GetPeaksDllLite.Functions.ConvertXYData.DeconXYDataToOmicsXYData(MassSpectrum);
       ...
       //     writer.toDiskStringList(outputPeaksFileName, allPeaks);
            ...
            GetPeaksDllLite.DataFIFO.StringListToDisk summarywriter = new GetPeaksDllLite.DataFIFO.StringListToDisk();
