...
namespace IQGlyQ.FIFO
{
    public class GlyQIqResult
    {
        ...
        //D/H
        public string MixingFraction { get; set; }
        public string DHRatio { get; set; }
        public string C13Abundance { get; set; }
        public List<string> isotopes { get; set; }

        public GlyQIqResult()
        {
            isotopes = new List<string>();
        }
        ...
        public GlyQIqResult(string[] resultWords)
        {
            ...
            {
                if(resultWords.Length > 45)//includes isotopes
                {
                    isotopes = new List<string>();
                    ...
                    InterferenceValue = resultWords[43];
                    ChargeCorrelation = resultWords[44];
                    MixingFraction = resultWords[45];
                    DHRatio = resultWords[46];
                    C13Abundance = resultWords[47];
                    for(int i=48;i<resultWords.Length;i++)
                    {
                        isotopes.Add(resultWords[i]);
                    }
                }
                else
                {
                    Console.WriteLine("MissingColumn in GlyQIqResult constructor (should be at least 46 data columns)");
        ...
        public string GlyQIqResultToString(string delim="\t")
        {
            ...
            else
            {
                ...
                resultAsString += IsoFit; resultAsString += delim;
                resultAsString += InterferenceValue += delim;
                resultAsString += ChargeCorrelation+= delim;
                resultAsString += MixingFraction += delim;
                resultAsString += DHRatio += delim;
                resultAsString += C13Abundance += delim;

                string buildingString = String.Join(delim, isotopes.ToArray());
                
                resultAsString += buildingString;//no delim
            }
