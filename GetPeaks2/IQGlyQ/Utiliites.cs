...
namespace IQGlyQ
{
    public static class Utiliites
    {
        ...
        {
            Run32.HelloWorld.Check();
            IQ2.HelloWorld.Check();
        ...
        public static Run32.Backend.Data.XYData GenerateBufferedChromatogramAbolute(PeakChromatogramGenerator _peakChromGen, Run runIn, ScanObject scans, double massToExtract, double chromTollerencePPM)
        {
        ...
        {
            List<Peak> outputDeconResults = new List<Run32.Backend.Core.Peak>();
            foreach (var peak in deconData)
            {
                ...
                {
                    outputDeconResults.Add((Run32.Backend.Core.Peak)peak);
                }
                else
                {
        ...
        /// <returns></returns>
        public static Run32.Backend.Data.XYData MovingAverage(Run32.Backend.Data.XYData deconXYData, int averageOddNumberOfPoints)
        {
            int deconXYDatalength = deconXYData.Xvalues.Length;
            double[] yValues = deconXYData.Yvalues;

            var newdeconXYData = new Run32.Backend.Data.XYData();
        ...
        public static List<Run32.Backend.Core.Peak> FilterByPointsPerPeak(IqResult result, int minNumberOfPoints)
        {
            var chromPeakList = new List<Run32.Backend.Core.Peak>();
        ...
        public static List<Run32.Backend.Core.Peak> FilterChromatogramPeaksByWidth(List<Run32.Backend.Core.Peak> deconPeaks, int scanSpacing, int minNumberOfPoints)
        {
            bool test1 = SignPostRequire(deconPeaks != null, "XYdataList is null");//perhaps more


            var outputResults = new List<Run32.Backend.Core.Peak>();
            if (test1)
            {
                foreach (var peak in deconPeaks)
                {
                    ...
                    {
                        outputResults.Add((Run32.Backend.Core.Peak)peak);
                    }
                    else
                    {
                        Console.WriteLine("Fail");
            ...
            {
                outputResults = new List<Run32.Backend.Core.Peak>();
            }
        ...
        public static ScanSet ScanSetFromStartStop(Run runIn, ScanObject scanInfo)
        {
            ...
            Run32.Backend.Core.Peak chromPeakSelected = new Run32.Backend.Core.Peak();
