    ...
    public class FragmentedAboveBelowParentIQWorkflow : IqWorkflow
    {
        ...
        public void ExecuteOld(IqResult result)
        {
            ...
            if (result.HasChildren())
            {
                ...
                List<int> scanList = (from n in validGlyQiqChildren select n.ToChild.Primary_Target.ScanLCTarget).Distinct().ToList();
                ...
                foreach (int scan in scanList)
                {
                    ...
                    List<IqGlyQResult> scanResultsList = (from n in validGlyQiqChildren where n.ToChild.Primary_Target.ScanLCTarget == scanCurrent select n).ToList();//perhaps add a range of scansets
                    
                    //this just has targets
                    List<IqGlyQResult> scanResultsListFragments = (from n in validGlyQiqChildrenTargets where n.ToChild.Primary_Target.ScanLCTarget == scanCurrent select n).ToList();//perhaps add a range of scansets
                    ...
                    if (globalResultForThisScanChild != null && globalResultForThisScanChild.ToChild.ChargeMonoMass.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                int maxCharge = mostAbundantInScan.ToChild.Primary_Target.ChargeState;
                                float maxAbundance = mostAbundantInScan.ToChild.FragmentFitAbundance;

                                isomerResult.ToChild.Primary_Target.ChargeState = maxCharge;
                                baseCallTarget.Abundance = maxAbundance;
                                if (mostAbundantInScan.ToChild.Primary_Observed_IsotopeProfile != null)
                                {
                                    baseCallTarget.MZObs = mostAbundantInScan.ToChild.Primary_Observed_IsotopeProfile.MonoPeakMZ;
                                    baseCallTarget.MonoMassObs = mostAbundantInScan.ToChild.Primary_Observed_IsotopeProfile.MonoIsotopicMass;
                                }
                            }
                    ...
                    if (scanResultsListFragments != null && scanResultsListFragments.Count > 0)
                    {
                        ...
                        if (avarageMassCounter > 0)
                        {
                            ...
                            double calibratedTheoreticalMZ = ConvertMonoToMz.Execute(isomerResult.ToChild.Primary_Target.MonoMassTheor,charge,massProton) + WorkflowParameters.MSParameters.IsoParameters.DeltaMassCalibrationMZ;
                            double calibratedTheoreticalMono = ConvertMzToMono.Execute(calibratedTheoreticalMZ,charge,massProton);
        ...
        {        
            ...
            List<int> chargeStateList = (from n in childrenOfTheScan where n.ToChild.TypeOfResultTargetOrModifiedTarget == TypeOfResult.Target select n.ToChild.Primary_Target.ChargeState).Distinct().ToList();//now we look at the charge state rangge of the targets only
            ...
            foreach (int charge in chargeStateList)
            {
                ...
                {
                    if (n.ToChild.Primary_Observed_IsotopeProfile != null)
