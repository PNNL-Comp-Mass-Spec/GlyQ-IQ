...
namespace IQGlyQ.Workflows
{
    public class ParentIQWorkflowWithChargeCorrelation : IqWorkflow
    {
        ...
        //this is the current front runner parent workflow
        public override void Execute(IqResult result)
        {
            ...
            if (result.HasChildren())
            {
                ...
                validGlyQiqChildren = validGlyQiqChildren.OrderBy(n => n.ToChild.Primary_Target.ScanLCTarget).ToList();
                ...
                List<int> scanList = (from n in validGlyQiqChildren where n.ToChild.TypeOfResultParentOrChildDifferenceApproach == EnumerationParentOrChild.ChildrenOnly select n.ToChild.Primary_Target.ScanLCTarget).Distinct().ToList();
                scanList.Sort();
                ...
                for (int i = 0; i < numberOfDistinctScans;i++ )
                {
                    ...
                    if (detailedScanList[i].isConsolidated == false)//this means it is that the charge states associated with this scan are clean.  Else, the charge states from multiple scans needs to be considered
                    {
                        ...
                        isomerResult.ToChild.Primary_Target.ScanLCTarget = scanCurrent;

                        //List<IqResult> tempCastGlyQIQResult = result.ChildResults().ToList();
                        //initial population of values
                        isomerResult.ToChild.Primary_Observed_IsotopeProfile.Score = detailedScanList[i].FitScoreFromMaxAbundance;
                        ...
                        List<IqGlyQResult> scanResultsListFragments = new List<IqGlyQResult>();//results from primary targets
                        List<IqGlyQResult> scanResultsList = new List<IqGlyQResult>();//all "valid"
                        List<IqGlyQResult> scanResultsListModfied = new List<IqGlyQResult>();//results with modified masses
                        ...
                        if(detailedScanList[i].scansToConsolidate.Count>0)//this means the scans have been identified that need to be consolidated  1000 and 1002 for example. 1000 came from +2 and 1001 came from +1  
                        {
                            ...
                            {
                                scanResultsListFragments.AddRange((from n in validGlyQiqChildrenTargets where n.ToChild.Primary_Target.ScanLCTarget == scan select n).ToList());//perhaps add a range of scansets
                                scanResultsList.AddRange((from n in validGlyQiqChildren where n.ToChild.Primary_Target.ScanLCTarget == scan select n).ToList());//perhaps add a range of scansets
                                scanResultsListModfied.AddRange((from n in validGlyQiqChildrenModfied where n.ToChild.Primary_Target.ScanLCTarget == scan select n).ToList());
                        ...
                        {
                            scanResultsListFragments = (from n in validGlyQiqChildrenTargets where n.ToChild.Primary_Target.ScanLCTarget == scanCurrent select n).ToList();
                            scanResultsList = (from n in validGlyQiqChildren where n.ToChild.Primary_Target.ScanLCTarget == scanCurrent select n).ToList();
                            scanResultsListModfied = (from n in validGlyQiqChildrenModfied where n.ToChild.Primary_Target.ScanLCTarget == scanCurrent select n).ToList();
                        }
                        ...
                        if (childScanResultsListFragments.Count>1)
                        {
                            ...
                            {
                                isomerResult.ToChild.Primary_Observed_IsotopeProfile.Score = correlate1.ToChild.Primary_Observed_IsotopeProfile.Score;
                                isomerResult.ToChild.LMOptimizationCorrelationRsquared = correlate1.ToChild.LMOptimizationCorrelationRsquared;
                            }
                            else
                            {
                                isomerResult.ToChild.Primary_Observed_IsotopeProfile.Score = correlate2.ToChild.Primary_Observed_IsotopeProfile.Score;
                        ...
                        if (globalResultForThisScanChild != null && globalResultForThisScanChild.ToChild.ChargeMonoMass.Count > 0)//dictionary has multiple monos
                        {
                            ...
                            {
                                int maxCharge = mostAbundantFromChildrenInScan.ToChild.Primary_Target.ChargeState;
                                float maxAbundance = mostAbundantFromChildrenInScan.Abundance;
                                
                                isomerResult.ToChild.Primary_Target.ChargeState = maxCharge;
                            ...
                            else
                            {
                                ...
                                //else//when nothing is found
                                {
                                    ...
                                    isomerResult.ToChild.Primary_Target.ChargeState = isomerResult.ToChild.GlobalChargeStateMax;//these have the same effect.  both charge and framgentcharge
                                    baseCallTarget.Abundance = 0;
                                    if (scanResultsListFragments != null && scanResultsListFragments.Count > 0)
                                    {
                                        baseCallTarget.MZObs = scanResultsListFragments[0].ToChild.Primary_Observed_IsotopeProfile.MonoPeakMZ;
                                        baseCallTarget.MonoMassObs = scanResultsListFragments[0].ToChild.Primary_Observed_IsotopeProfile.MonoIsotopicMass;
                        ...
                        if (scanResultsListFragments != null && scanResultsListFragments.Count > 0)
                        {
                            ...
                            foreach (IqGlyQResult searchForMonoMassResult in scanResultsListFragments)
                            {
                                ...
                                double monoMass = searchForMonoMassResult.ToChild.Primary_Observed_IsotopeProfile.MonoIsotopicMass;
                                if (monoMass > 0)
                                {
                            ...
                            if (avarageMassCounter > 0)
                            {
                                ...
                                double existingTheoreticalMonoMass = isomerResult.ToChild.Primary_Target.MonoMassTheor;//this is for MZ shift only because Mono is allready calibrated

                                double calibratedTheoreticalMono;//for new ppm calculation
                                double nonCalibratedMono = isomerResult.ToChild.Primary_Target.MonoMassTheor;
                                if (WorkflowParameters.MSParameters.IsoParameters.ToMassCalibrate)
                                {

                                    int charge = isomerResult.ToChild.Primary_Target.ChargeState;
                                ...
                                Console.WriteLine("old Mono mass theoretical was: " + isomerResult.ToChild.Primary_Target.MonoMassTheor + " calibrated is now: " + calibratedTheoreticalMono);

                                double oldPPM = ErrorCalculator.PPM(isomerResult.ToChild.AverageMonoMass, isomerResult.ToChild.Primary_Target.MonoMassTheor);
                        ...
                        if(ifDH)
                        {
                            ...
                            List<IqGlyQResult> results_Children_Intensity = results_Children.OrderByDescending(n => n.ToChild.GlobalAggregateMSAbundance).ToList();
                            IqGlyQResult results_Children_Intensity_MostAbundant = results_Children_Intensity.FirstOrDefault();
                            IsotopicProfile bestTargetProfile = results_Children_Intensity_MostAbundant.ToChild.Primary_Target.TheorIsotopicProfile;
                            IsotopicProfile bestObservedProfile = results_Children_Intensity_MostAbundant.ToChild.Primary_Observed_IsotopeProfile;

                            isomerResult.ToChild.Primary_Target.TheorIsotopicProfile = bestTargetProfile.CloneIsotopicProfile();
                            isomerResult.ToChild.Primary_Target.TheorIsotopicProfile.EstablishAlternatvePeakIntensites(bestTargetProfile.EstablishedMixingFraction);

                            isomerResult.ToChild.Primary_Observed_IsotopeProfile = bestObservedProfile.CloneIsotopicProfile();
                            isomerResult.ToChild.Primary_Observed_IsotopeProfile.EstablishAlternatvePeakIntensites(bestObservedProfile.EstablishedMixingFraction);
                            
                        }

                        //        globalResultForThisScan.ToChild.Primary_Observed_IsotopeProfile.AbundanceC13asFraction =
                //globalResultForThisScan.ToChild.Primary_Observed_IsotopeProfile.EstablishAlternatvePeakIntensites(5);
                //        globalResultForThisScan.ToChild.Primary_Observed_IsotopeProfile.Score = 5;
                //        globalResultForThisScan.ToChild.Primary_Observed_IsotopeProfile.Peaklist = new List<MSPeak>();
        ...
        private static void PrintIqGlyQResultList(List<IqGlyQResult> scanResultsListFragments)
        {
            ...
            {
                Console.WriteLine("Scan: " + iqGlyQResult.ToChild.Primary_Target.ScanLCTarget.ToString() +
                                  ", TargetType: " + iqGlyQResult.ToChild.TypeOfResultTargetOrModifiedTarget +
                                  ", Charge: " + iqGlyQResult.ToChild.Primary_Observed_IsotopeProfile.ChargeState +
        ...
        private static double CorrelateModelsFromDifferentScans(SimplePeakCorrelator correlator, List<IqGlyQResult> validGlyQiqChildren, int candidateScan, int centerScan)
        {
            List<IqGlyQResult> scanResultsListCenterScan = (from n in validGlyQiqChildren
                                                            where n.ToChild.Primary_Target.ScanLCTarget == centerScan && 
            ...
            if (baseToCorrelateTo != null)
            {
                ...
                List<IqGlyQResult> resultsListFromLowerScan = (from n in validGlyQiqChildren
                                                               where n.ToChild.Primary_Target.ScanLCTarget == candidateScan && 
                                                               n.ToChild.TypeOfResultTargetOrModifiedTarget == TypeOfResult.Target &&
                                                               n.ToChild.TypeOfResultParentOrChildDifferenceApproach == EnumerationParentOrChild.ChildrenOnly
                                                               select n).ToList(); //perhaps add a range of scansets
        ...
        private static List<float> SetMaxAbundancePerDistinctScan(List<IqGlyQResult> validGlyQiqChildren, List<int> scanList, out List<double> scanListMaxAbundanceFitScores, out List<double> scanListMaxAbundanceLMFit, out List<float> msAbundances)
        {
            ...
            for (int i = 0; i < numberOfDistinctScans; i++)
            {
                ...
                List<IqGlyQResult> scanResultsList = (from n in validGlyQiqChildren where n.ToChild.Primary_Target.ScanLCTarget == scanCurrent select n).ToList();
                ...
                if (scanResultsList[0].ToChild.Primary_Observed_IsotopeProfile != null) currentFitScore = scanResultsList[0].ToChild.Primary_Observed_IsotopeProfile.Score;
                ...
                foreach (var iqGlyQResult in scanResultsList)
                {
                    if (iqGlyQResult.ToChild.FragmentFitAbundance > abundance)
                    {
                        ...
                        if (iqGlyQResult.ToChild.Primary_Observed_IsotopeProfile != null) currentFitScore = scanResultsList[0].ToChild.Primary_Observed_IsotopeProfile.Score;
        ...
        {        
            ...
            List<int> chargeStateList = (from n in targets select n.ToChild.Primary_Target.ChargeState).Distinct().ToList();//now we look at the charge state rangge of the targets only
            ...
            foreach (int charge in chargeStateList)
            {
                ...
                foreach (IqGlyQResult n in targets)
                {
                    ...
                    if (n.ToChild.Primary_Observed_IsotopeProfile != null && n.ToChild.Primary_Observed_IsotopeProfile.ChargeState == charge)
                    {
                        monoMass = n.ToChild.Primary_Observed_IsotopeProfile.MonoIsotopicMass;
        ...
        public override ResultExporter CreateExporter()
        {
            ...
            if(_workflowParameters.MSParameters.IsoParameters.IsotopeProfileMode == EnumerationIsotopicProfileMode.DH  )
            {
                exporter.ToPrintDH = true;
                exporter.NumberOfPenaltyIons = 1;
                exporter.NumberOfIsotopesToPrint = 8;
            }
            return exporter;
