...
namespace GetPeaksDllLite.Functions
{
    ...
    {
        public static List<PNNLOmics.Data.Peak> Convert(List<Run32.Backend.Core.MSPeak> deconPeakList, bool keepPeaks)
        {
            List<PNNLOmics.Data.Peak> omicsPeakList = new List<PNNLOmics.Data.Peak>();

            for (int i = 0; i < deconPeakList.Count;i++)
            {
                Run32.Backend.Core.MSPeak dPeak = deconPeakList[i];
        ...
        public static List<PNNLOmics.Data.Peak> ConvertPeak(List<Run32.Backend.Core.Peak> deconPeakList, bool keepPeaks)
        {
            List<PNNLOmics.Data.Peak> omicsPeakList = new List<PNNLOmics.Data.Peak>();

            for (int i = 0; i < deconPeakList.Count; i++)
            {
                Run32.Backend.Core.Peak dPeak = deconPeakList[i];
        ...
        public static List<ProcessedPeak> ConvertToProcessedPeak(List<Run32.Backend.Core.Peak> deconPeakList, bool keepPeaks)
        {
