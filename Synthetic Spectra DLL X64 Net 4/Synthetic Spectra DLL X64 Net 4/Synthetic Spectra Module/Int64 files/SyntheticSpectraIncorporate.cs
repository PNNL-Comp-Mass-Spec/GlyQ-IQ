...
{
    public class SyntheticSpectraIncorporate
    {
        public List<PeakGeneric<Int64, double>> AllignNoise(List<PeakGeneric<Int64, double>> spectra, List<XYDataGeneric<Int64, double>> noiseSpectra, Int64 peak_spacing)
        {
            //returns a spectra interpoated onto the grid

            List<PeakGeneric<Int64, double>> gridSpectra = new List<PeakGeneric<Int64, double>>();  //where we store the new intensities
            ...
            //CompareController CompareContrastX = new CompareController();
            //CompareContrastX.CompareOnlyInt(splineIndexes, massIndexes, Masses, spectra, noiseSpectra);

            SyntheticSpectraIncorporate CompareNew = new SyntheticSpectraIncorporate();
            CompareNew.CompareOnlyInt(splineIndexes, massIndexes, Masses, spectra, noiseSpectra);
            ...
            {
                PeakGeneric<Int64, double> NewPeak = new PeakGeneric<Int64, double>();
                NewPeak.Mass = spectra[i].Mass;//a =1 offset is correct

                if (i == massIndexes[counter])
                {
                    Masses[counter] = Masses[counter];
                    addedValue = (Int64)method2.Interpolate((double)NewPeak.Mass);
                    NewPeak.Intensity = (double)addedValue;
        ...
        public void CompareOnlyInt(List<int> libraryIndexes, List<int> massIndexes, List<Int64> masses, List<PeakGeneric<Int64, double>> spectraMasses, List<XYDataGeneric<Int64, double>> spectraLibrary)
        {
            int i = 0;
            int j = 0;
            ...
            {
                XYData XY = new XYData();
                XY.X = (double)spectraLibrary[i].X;
                massList2.Add(XY);
            }
