...
using MSPeakResult = Run32.Backend.DTO.MSPeakResult;
using System.Linq;
...
namespace IQ.Workflows.FileIO.Importers
{
    ...
    public class PeakImporterFromText : IPeakImporter
    {
        ...
        public override void ImportPeaks(List<Run32.Backend.DTO.MSPeakResult> peakList)
        {
            using (StreamReader reader = new StreamReader(filename))
            {
        ...
        private MSPeakResult convertTextToPeakResult(string line)
        {
            ...
            var newMSPeak = new MSPeak();

            //UIMF peak data contains an extra column
            if (_peaksAreFromUIMF) ++columnCounter;

            newMSPeak.XValue = Convert.ToDouble(processedLine[++columnCounter]);
            newMSPeak.Height = Convert.ToSingle(processedLine[++columnCounter]);
            newMSPeak.Width = Convert.ToSingle(processedLine[++columnCounter]);
            newMSPeak.SignalToNoise = Convert.ToSingle(processedLine[++columnCounter]);

            peakresult.MSPeak = newMSPeak;
        ...
        {
            return inputLine.Split(delimiter).ToList<string>();
        }
    }
}
