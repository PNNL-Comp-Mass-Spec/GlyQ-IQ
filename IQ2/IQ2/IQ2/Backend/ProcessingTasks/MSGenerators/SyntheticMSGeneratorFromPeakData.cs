...
namespace IQ.Backend.ProcessingTasks.MSGenerators
{
            ...
            foreach (MSPeak peak in run.PeakList)
            {
                ...
                switch (ModeOfPeakWidthCalculation)
                {
                    case SyntheticMSGeneratorFromPeakDataMode.WidthsCalculatedFromSingleValue:
                        ...
                        generatedXYData = TheorXYDataCalculationUtilities.GetTheorPeakData(this.PeakWidthForAllPeaks, peak.Height,peak.Width, 11);
                        break;
                    case SyntheticMSGeneratorFromPeakDataMode.WidthsCalculatedOnAPerPeakBasis:
                        ...
                        generatedXYData = TheorXYDataCalculationUtilities.GetTheorPeakData(peak.Width, peak.Height, peak.Width, 11);
                        break;
                    default:
                        //generatedXYData = peak.GetTheorPeakData(this.PeakWidthForAllPeaks, 11);
                        generatedXYData = TheorXYDataCalculationUtilities.GetTheorPeakData(this.PeakWidthForAllPeaks, peak.Height, peak.Width, 11);
