...
{
    public class PeakChromatogramGenerator : TaskIQ
    {
        int maxZerosToAdd = 2;
        ...
        private XYData getChromValues(List<Run64.Backend.DTO.MSPeakResult> filteredPeakList, Run run)
        {
            XYData data = new XYData();

            SortedDictionary<int, double> xyValues = new SortedDictionary<int, double>();
            foreach (Run64.Backend.DTO.MSPeakResult peak in filteredPeakList)
            {
        ...
        public XYData getChromValues2(List<Run64.Backend.DTO.MSPeakResult> filteredPeakList, Run run)
        {
