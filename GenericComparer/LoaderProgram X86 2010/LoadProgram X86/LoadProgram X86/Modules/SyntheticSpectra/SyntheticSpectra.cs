...
namespace ConsoleApplication1
{
	public class SyntheticSpectra
	{
        ...
        public List<PeakDecimal> SynSpectraGrid(ref SyntheticSpectraParameters Parameters, PrintBuffer print)
		
        {
			//create a list of equal spaced points
			List<PeakDecimal> synSpectra = new List<PeakDecimal>();

			print.AddLine("the Range is " + Parameters.StartMass + " to " + Parameters.EndMass);

            decimal expectedNumberOfPoints = (decimal)((Parameters.EndMass - Parameters.StartMass) / Parameters.PeakSpacing);
			print.AddLine("the Size " + expectedNumberOfPoints + " or  ");

			int numberOfPoints = (int)Math.Floor(expectedNumberOfPoints);

			print.AddLine("reduced " + numberOfPoints);
		...
		public void AddMass(List<PeakDecimal> spectra, decimal mass, double intensity,string isotopeType, ref SyntheticSpectraParameters Parameters, PrintBuffer print)
        {
			...
			List<PeakDecimal> isotopePattern = new List<PeakDecimal>();
			...
			for (int j = 0; j < 8; j++)  //add up to 8 isotopes
			{
                ...
                newSpectra.AddIon(spectra, massTemp, intensityTemp, ref Parameters, print);
			}
		}
		...
		//public void AddIon(List<PeakDecimal> spectra, decimal mass, double intensity, decimal resolution, decimal peakSpacing, double percentHanning, PrintBuffer print)
        {
			...
			print.AddLine("Mass in " + mass + ". Closest Mass is "+ spectra[index].Mass);
			...
			double yMax = modelFX(xMax, mass, intensity, resolution, percentHanning, print);
			...
			for (int i = 0; i < startPoint * 2+1; i++)
			{
				...
				double y = modelFX(x, mass, intensity, resolution, percentHanning, print);

				if ((!Double.IsNaN(y)))
				{
					x = -startPoint * peakSpacing + i * peakSpacing + offsetX;
					y = modelFX(x, mass, intensity, resolution, percentHanning, print);
				}
				else
				{	
			...
			print.AddLine("the mass is " + mass);
		...
		private double modelFX(decimal xDecimal, decimal centerMass, double intensity, double resolution, double percentHanning, PrintBuffer print)
		{
