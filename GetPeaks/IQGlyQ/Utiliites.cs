...
namespace IQGlyQ
{
    public static class Utiliites
    {
        ...
        public static DeconTools.Backend.XYData GenerateBufferedChromatogramAbolute(PeakChromatogramGenerator _peakChromGen, Run runIn, ScanObject scans, double massToExtract, double chromTollerencePPM)
        {
            ...
            DeconTools.Backend.XYData deconChromatogram = ProcessorChromatogram.DeconChromatogramGeneratorWrapper(runIn, massToExtract, bufferedRange);
        ...
        public static IsotopicProfile GenerateCombinedIsotopicProfile(IqResult result, int isotopeOffset, float ratioProfile2)
        {
            ...
            comboProfile = GenerateCombinedIsotopicProfile(result.Target.TheorIsotopicProfile, isotopeOffset, ratioProfile2);

            return comboProfile;
        }

        public static IsotopicProfile GenerateCombinedIsotopicProfile(IsotopicProfile isotopeProfile, int isotopeOffset, float ratioProfile2)
        {
            IsotopicProfile comboProfile = new IsotopicProfile();
            ...
            comboProfile = isotopeProfile.CloneIsotopicProfile();
            comboProfile.Peaklist = comboPeakList;
        ...
        public static void RenewChromPeakStartStop(ref Run runIn, FragmentedTargetedWorkflowParametersIQ workflowParameters, ref Processors.ProcessorChromatogram LcProcessor, FragmentResultsObjectIq processedTargetResult, ref IqGlyQResult iQResult, ref ScanObject peakQualityScanBounds, ref Tuple<string, string> errorlog)
        {
            ...
            if (iQResult.Target.TheorIsotopicProfile != null)
            {
                ...
                double currentMZMostAbundant = iQResult.Target.TheorIsotopicProfile.Peaklist[iQResult.Target.TheorIsotopicProfile.GetIndexOfMostIntensePeak()].XValue;
                ...
                DeconTools.Backend.XYData deconBufferedChromatogram = LcProcessor.GenerateBufferedChromatogramByPoint(runIn, peakQualityScanBounds, currentMZMostAbundant);
                
                
                List<XYData> omicsBufferedChromatogram = ConvertXYData.DeconXYDataToOmicsXYData(deconBufferedChromatogram);
                ...
                List<ProcessedPeak> renewFeature = LcProcessor.Execute(omicsBufferedChromatogram, peakQualityScanBounds, workflowParameters.LCParameters.ProcessLcChromatogram);

                //4.  select desired list from mini list of features
                if (renewFeature.Count > 1)
                {
        ...
        private static SolverReport FitWith2SeedXValues(double LM_RsquaredCuttoff, out List<XYData> clippedFitXyData, ref Tuple<string, string> errorLog, List<XYData> possibleEicSinglePeak, out ScanObject scanRangeFit,out List<XYData> modeledPeakList, out double areaUnderCurve, out double[] coefficents)
        {
            ...
            Console.WriteLine("      Fit Seed 1");
            clippedFitXyData = CurveFit.Fit_LM(possibleEicSinglePeak, out fitMetrics, out areaUnderCurve, ref coefficents,out modeledPeakList, numberOfSamples, sampleWidth, ref errorLog, out scanRangeFit,EnumerationCurveType.Gaussian);
            Console.WriteLine("      Fit Seed 1 converged? " + fitMetrics.DidConverge);
            ...
            if (fitMetrics.RSquared < LM_RsquaredCuttoff && possibleEicSinglePeak.Count > 0 && fitMetrics.DidConverge == true || fitMetrics.DidConverge == false && possibleEicSinglePeak.Count > 0)//if fit is already good, don't refit.  we need possibleEicSinglePeak points present inorder to refit OR if it did not converge and we have peaks, procede with seed 2
            {
                ...
                Console.WriteLine("      Fit Seed 2");
                SolverReport fitMetricsSeed2;
                clippedFitXyData = CurveFit.Fit_LM(possibleEicSinglePeak, out fitMetricsSeed2, out areaUnderCurve,ref coefficents, out modeledPeakList, numberOfSamples, sampleWidth,ref errorLog, out scanRangeFit, EnumerationCurveType.Gaussian);
                Console.WriteLine("      Fit Seed 2 converged? " + fitMetricsSeed2.DidConverge);
        ...
        private static double CutAndRefitCurve(ref List<PNNLOmics.Data.XYData> XYDataToFit, ref Tuple<string, string> errorLog, int minNumberOfPointsToAllow, double LM_RsquaredCuttoff, List<PNNLOmics.Data.XYData> possibleFragmentTargetEicSinglePeak, ref SolverReport fitMetrics, out ScanObject scanRangeFit, out double[] coefficents, out List<PNNLOmics.Data.XYData> fitXYDataForReturn)
        {
            ...
            if (XYDataToFit.Count > minNumberOfPointsToAllow)
            {
                ...
                while (continueLoop)
                {
                    ...
                    //remove lowest
                    Console.WriteLine("*****TryAgain***** " + XYDataToFit.Count + " point  " + fitMetrics.RSquared + " rsquared/" + LM_RsquaredCuttoff);
        ...
        {
            List<Peak> outputDeconResults = new List<DeconTools.Backend.Core.Peak>();
            foreach (var peak in deconData)
            {
                ...
                {
                    outputDeconResults.Add((DeconTools.Backend.Core.Peak) peak);
                }
                else
                {
        ...
        /// <returns></returns>
        public static DeconTools.Backend.XYData MovingAverage(DeconTools.Backend.XYData deconXYData, int averageOddNumberOfPoints)
        {
            int deconXYDatalength = deconXYData.Xvalues.Length;
            double[] yValues = deconXYData.Yvalues;

            DeconTools.Backend.XYData newdeconXYData = new DeconTools.Backend.XYData();
        ...
        public static List<DeconTools.Backend.Core.Peak> FilterByPointsPerPeak(IqResult result, int minNumberOfPoints)
        {
            List<DeconTools.Backend.Core.Peak> chromPeakList = new List<DeconTools.Backend.Core.Peak>();
        ...
        public static List<DeconTools.Backend.Core.Peak> FilterChromatogramPeaksByWidth(List<DeconTools.Backend.Core.Peak> deconPeaks, int scanSpacing, int minNumberOfPoints)
        {
            bool test1 = SignPostRequire(deconPeaks != null, "XYdataList is null");//perhaps more


            List<DeconTools.Backend.Core.Peak> outputResults = new List<DeconTools.Backend.Core.Peak>();
            if (test1)
            {
                foreach (var peak in deconPeaks)
                {
                    ...
                    {
                        outputResults.Add((DeconTools.Backend.Core.Peak)peak);
                    }
                    else
                    {
                        Console.WriteLine("Fail");
            ...
            {
                outputResults = new List<DeconTools.Backend.Core.Peak>();
            }
        ...
        public static FragmentIQTarget GenerateListOfCandidateParentsTestIQ(Run run, IqResult result, ChromPeakQualityData possibiity, ScanObject scans, EnumerationParentOrChild parentsOrChildren, ref FragmentedTargetedWorkflowParametersIQ _workflowParameters, ref ITheorFeatureGenerator _theorFeatureGen, ref Tuple<string, string> errorLog, ref Processors.ProcessorChromatogram _lcProcessor)
        {
            ...
            if (test1 && test2 && test3 && test4)
            {
                ...
                chargedParentsToSearchForMass = GenerateListOfCandidateParents.GenerateIQ(run, result, possibiity, scans, parentsOrChildren, out failedTargets, out moreFailedParents, ref _workflowParameters, ref _theorFeatureGen, ref errorLog, ref _lcProcessor);
        ...
        public static ScanSet ScanSetFromStartStop(Run runIn, ScanObject scanInfo)
        {
            ...
            DeconTools.Backend.Core.Peak chromPeakSelected = new DeconTools.Backend.Core.Peak();
        ...
        public static void TherortIsotopeicProfileWrapper(ref ITheorFeatureGenerator TheorFeatureGen, IqTarget target, EnumerationIsotopicProfileMode switchOption, double deltaMassCalibrationMZ, double deltaMassCalibrationMono, bool toMassCalibrate, EnumerationIsotopePenaltyMode penaltySwitch)
        {
           //step 1 is generated a non calibrated non modified iso
            ...
            switch (switchOption)
            {
                case EnumerationIsotopicProfileMode.H:
                    {
                        target.TheorIsotopicProfile = TherortIsotopeicProfileWrapperLowLevel(ref TheorFeatureGen, target.EmpiricalFormula, target.ChargeState);
                    }
                    break;
                ...
                case EnumerationIsotopicProfileMode.DH:
                    {
                        ...
                        //then add a second distribution ontop
                        target.TheorIsotopicProfile = Utiliites.GenerateCombinedIsotopicProfile(target.TheorIsotopicProfile, 1, 1);
                    }
                    break;
                default:
                    {
            ...
            if (toMassCalibrate)
            {
                MassCalibration.Iso(ref theorIsotopicProfile, deltaMassCalibrationMZ, deltaMassCalibrationMono);
            }
                ...
                case EnumerationIsotopePenaltyMode.PointToLeftAndHarmonic:
                    {
                        ...
                        int indexToAdd = 1;//0 will put a peak between 0 and 1
                        AddHarmonic(indexToAdd, theorIsotopicProfile);
                    }
                    break;
            }
        ...
        private static void AddHarmonic(int indexToAdd, IsotopicProfile theorIsotopicProfile)
        {
            double peakSpacing = theorIsotopicProfile.Peaklist[1].XValue - theorIsotopicProfile.Peaklist[0].XValue;
            double newMass = theorIsotopicProfile.Peaklist[indexToAdd].XValue + (peakSpacing/2);
                //place a new point right between two peaks. this should be extended to the most abundant location
            theorIsotopicProfile.Peaklist.Insert(0, new MSPeak(newMass, 0, 0, 0));
        }

        private static void AddFirstPeak(IsotopicProfile theorIsotopicProfile)
        {
            if (theorIsotopicProfile.Peaklist != null)
            {
                if (theorIsotopicProfile.Peaklist.Count == 1)
                {
                    double peakSpacing = 1;
                    double newMass = theorIsotopicProfile.Peaklist[0].XValue - (peakSpacing);
                    theorIsotopicProfile.Peaklist.Insert(0, new MSPeak(newMass, 0, 0, 0));
                }
                else
                {
        ...
        public static IsotopicProfile TherortIsotopeicProfileWrapperLowLevel(ref ITheorFeatureGenerator theorFeatureGen, string empiricalFormula, int chargeState)
        {
        ...
        public static List<IqGlyQResult> VerifyByTargetedFeatureFindingTestIQ(FragmentIQTarget finalChargedParentsToSearchForMass, double fitScoreCutoff, Run run, ref IterativeTFF _msfeatureFinder, ref List<FragmentIQTarget> _futureTargets, IsotopeParameters fitScoreParameters, Processors.ProcessorMassSpectra msProcessor, ref Tuple<string, string> errorLog)
            ...
            
        {
            ...
            if (test1 == true && test2 == true)
            {
                ...
                finalChargedParentsToSearchForLC = VerifyByTargetedFeatureFinding.VerifyIq(finalChargedParentsToSearchForMass, fitScoreCutoff, run, out failedTargets, ref msProcessor, ref _msfeatureFinder, ref _futureTargets, ref errorLog, out verifyError, fitScoreParameters);
        ...
        public static bool TestProcessedPeak(ProcessedPeak fragmentCandiateFitPeak, List<XYData> fragmentEicFitXyData, FragmentIQTarget possibleFragmentTarget, FragmentResultsObjectHolderIq result, double fitscoreCuttoff)
        {
            ...
            if (fragmentCandiateFitPeak != null)
            {
                ...
                bool test6 = result.FragmentObservedIsotopeProfile !=null;
                bool test7 = true;
                bool test8 = true;
                bool test9 = true;
                if (test6)
                {
                    fitScore = result.FragmentObservedIsotopeProfile.Score;

                    test7 = result.FragmentObservedIsotopeProfile.MonoIsotopicMass > 0;

                    test8 = result.FragmentObservedIsotopeProfile.MonoPeakMZ > 0;
