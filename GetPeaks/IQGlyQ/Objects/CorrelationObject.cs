...
namespace IQGlyQ.Objects
{
    public class CorrelationObject
    {
        ...
        /// </summary>
        public List<DeconTools.Backend.XYData> PeakChromXYData { get; set; }
        ...
        public CorrelationObject(IsotopicProfile iso, ScanObject scans, Run run, ref ProcessorChromatogram _lcProcessor, FragmentedTargetedWorkflowParametersIQ workflowParameters)
        {
            ...
            PeakChromXYData = new List<DeconTools.Backend.XYData>();
            PeaksFromChromXYData = new List<List<ProcessedPeak>>();
            List<ProcessedPeak> peaksInEIC;
            DeconTools.Backend.XYData clippedEIC;
            foreach (MSPeak peak in iso.Peaklist)
            {
                ...
                else
                {
                    ...
                    if (clippedEIC==null) {clippedEIC = new DeconTools.Backend.XYData();}
        ...
        private DeconTools.Backend.XYData CreateBufferSmoothClip(ScanObject scans, Run run, ref ProcessorChromatogram lcProcessor, MSPeak peak, ProcessingParametersChromatogram LCparameters)
        {
            //1.  return a raw chromatorgram with buffers on each side
            DeconTools.Backend.XYData deconBufferedChromatogram = lcProcessor.GenerateBufferedChromatogramByPoint(run, scans,peak.XValue);
            ...
            //4.  convert back
            DeconTools.Backend.XYData finallyData = ConvertProcessedPeakToXYData.ConvertPointsToDecon(newMethodInfinity);
            return finallyData;
        }

        //slower and should produce the same results as the buffered version
        private DeconTools.Backend.XYData CreateFullSmoothClip(ScanObject scans, Run run, ref ProcessorChromatogram lcProcessor, MSPeak peak, ProcessingParametersChromatogram LCparameters)
        {
            //1.  return a raw chromatorgram with buffers on each side
            ...
            DeconTools.Backend.XYData deconBufferedChromatogram = ProcessorChromatogram.DeconChromatogramGeneratorWrapper(run, peak.XValue, scans);
        ...
        private bool CheckEIC(bool isThereAPeakInFront, int minSizeOfEIC)
        {
            ...
            {
                DeconTools.Backend.XYData EICofInterest = PeakChromXYData[i];
        ...
        private DeconTools.Backend.XYData FilterOutDataBasedOnMsMsLevel(Run run, DeconTools.Backend.XYData xyData, int msLevelToUse = 1, bool usePrimaryLcScanNumberCache = true)
        {
            if (xyData == null || xyData.Xvalues.Length == 0) return xyData;

            DeconTools.Backend.XYData filteredXYData = new DeconTools.Backend.XYData();
