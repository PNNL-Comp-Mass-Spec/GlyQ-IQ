...
namespace GetPeaks_DLL.Summing
{
    public class AllignAndSum3
    {
        ...
        {
            SortedDictionary<Int64, double> sortedPile = new SortedDictionary<Int64, double>();
            ...
            {
                if (DataToSum[0].PeakListY[i] > 0)
                {
                    sortedPile.Add(DataToSum[0].PeakListX64[i], DataToSum[0].PeakListY[i]);//initial adding since we are allready looping
                }
            ...
            #region bin data as quickly as possible
                ...
                foreach (XYDataAndPeakHolderObjectInt64 scanToSum in DataToSum)
                {
                    ...
                    for (int i = 0; i < scanToSum.PeakListX64.Count; i++)
                    {
                        if (scanToSum.PeakListX64[i] > 0)
                        {
                            if (sortedPile.ContainsKey(scanToSum.PeakListX64[i]))
                            //if (sortedPile.TryGetValue(scanToSum.PeakListX64[i],out tryGetkeyValue))
                            {
                                sortedPile[scanToSum.PeakListX64[i]] += scanToSum.PeakListY[i];
                            }
                            else
                            {
                                sortedPile.Add(scanToSum.PeakListX64[i], scanToSum.PeakListY[i]);//initial adding since we are allready looping
                            }
                        }
                    }
                }  
