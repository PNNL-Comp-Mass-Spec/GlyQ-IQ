...
namespace GetPeaks.UnitTests
{
    public static class YafmsDbUtilities
    {
        ...
        private static void RemapXYPeaksToCentricForWrite(List<XYData> simplePeaksHold, int scan, ref int IdCounterForPeakCentric, ref ScanCentric currentLocationInDataset, out List<PeakCentric> centroidedPeaksToWrite, out List<ScanCentric> centroidedScansToWrite)
        {
            centroidedPeaksToWrite = new List<PeakCentric>();
            centroidedScansToWrite = new List<ScanCentric>();
            ...
            foreach (XYData peak in simplePeaksHold)
            {
                PeakCentric newPeak = new PeakCentric();
                newPeak.Mz = peak.X;
